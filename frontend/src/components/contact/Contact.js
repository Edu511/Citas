...
export default class Contact extends Component {
	...
	listarCatalogos = () => {
		...
		// catalogoClasPersona
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/ClasificacionPersonas/Listar", {
				httpsAgent: agent,
			})
			.then((response) => {
		...
		// catalogoGenero
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/Generoes/Listar", { httpsAgent: agent })
			.then((response) => {
		...
		// catalogoDocIdentificacion
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/DocIdentificacions/Listar", {
				httpsAgent: agent,
			})
			.then((response) => {
		...
		// catalogoNacionalidad
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/Nacionalidads/Listar", {
				httpsAgent: agent,
			})
			.then((response) => {
		...
		// catalogoOcupacion
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/Ocupacions/Listar", { httpsAgent: agent })
			.then((response) => {
		...
		// catalogoNivelEstudios
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/NivelEstudios/Listar", {
				httpsAgent: agent,
			})
			.then((response) => {
		...
		// catalogoLengua
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/Lenguas/Listar", { httpsAgent: agent })
			.then((response) => {
		...
		// catalogoReligion
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/Religions/Listar", { httpsAgent: agent })
			.then((response) => {
		...
		// catalogoDiscapacidad
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/Discapacidads/Listar", {
				httpsAgent: agent,
			})
			.then((response) => {
		...
		// catalogoDelito
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/Delitoes/Listar", { httpsAgent: agent })
			.then((response) => {
		...
		// catalogoDistrito
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/Agencias/Listar", { httpsAgent: agent })
			.then((response) => {
				let distritos = [
					{
						distrito: response.data[10].nombreDistrito,
						id_distrito: response.data[10].distritoId,
						id_agencia: response.data[10].idAgencia,
					},
					{
						distrito: response.data[1].nombreDistrito,
						id_distrito: response.data[1].distritoId,
						id_agencia: response.data[1].idAgencia,
					},
				];
		...
		// catalogoEstado
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/Estadoes/Listar", { httpsAgent: agent })
			.then((response) => {
		...
		// catalogoMunicipio
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/Municipios/ListarPorEstado/" + 13, {
				httpsAgent: agent,
			})
			.then((response) => {
				let municipiosArray = response.data;
				this.setState({
					catalogoMunicipio: municipiosArray.sort((a, b) => {
		...
		// catalogoLocalidad
		axios
			.get("https://" + this.state.base_ip + ":" + this.state.puerto + "/api/Localidads/MostrarPorMPO/" + 710, {
				httpsAgent: agent,
			})
			.then((response) => {
				let localidadesArray = response.data;
				this.setState({
					catalogoLocalidad: localidadesArray.sort((a, b) => {
						return a.nombre === b.nombre ? 0 : a.nombre > b.nombre ? 1 : -1;
					}),
				});
			})
			...
			axios
				.get(
					"https://" +
						...
						":" +
						this.state.puerto +
						"/api/Municipios/ListarPorEstado/" +
	...
	cargarHorariosDisponibles = () => {
		...
		axios
			.post("https://" + ip_address + ":" + puerto + "/api/PreHorariosDisponibles/Listarpordia", datos_requeridos, {
				httpsAgent: agent,
			})
			.then((response) => {
	...
	checarDiaSemana = (event) => {
		this.setState({ dateFechaCita: event.target.value }, () => {
			...
			if ([6, 0].includes(dia_fecha)) {
				...
				this.setState(
					{
						dateFechaCita: fecha_seleccionada.getFullYear() + "-" + mes + "-" + dia,
					},
					() => {
	...
	enviarCorreo = () => {
		let correo = this.state.emailCorreo;
		let fecha = this.state.dateFechaCita || new Date().toISOString();
		let hora = this.state.selHorarioCita || "12:00";
		let agencia = this.state.selAgenciaAVisitar || "pachuca";

		console.log(correo, fecha, hora, agencia);

		let datos_correo_usuario = {
			correo: correo,
			asunto: "Datos de cita para atención temprana - PGJEH",
			mensaje:
				"Gracias por tu cooperación, se ha agenado una cita el dia " +
				fecha +
				" a las " +
				hora +
				"en las oficinas del Centro de Atencion Temprana " +
				agencia +
		...
		let datos_correo_acuse = {
			...
			asunto: "Nueva cita generada en el portal de denuncias PGJEH",
			mensaje:
				"Hola, se ha agendado una cita el dia " +
				fecha +
				" a las " +
				hora +
				"en las oficinas del Centro de Atencion Temprana " +
		...
		axios
			.post("http://localhost:5000/correo/enviar", datos_correo_usuario)
			.then((response) => {
				console.log(response);
				axios
					.post("http://localhost:5000/correo/enviar", datos_correo_acuse)
					.then((response) => {
						console.log(response);
					})
					.catch((error) => {
	...
	enviar(e) {
		e.preventDefault();
		var email_validate_response = this.validarEmail();
		...
		if (email_validate_response !== true) {
			...
			if (this.state.swAnonimo) {
				parametros = {
					...
					swDiscapacidad: false,
					selDiscapacidad: " ",
					selDelito: this.state.selDelito,
			...
			// si es menor a 18 años, se convierten en parametros protegidos
			...
			// asignación de distrito y agencia
			switch (this.state.selAgenciaAVisitar) {
				case "pachuca":
					id_distrito = this.state.catalogoDistritos[0].id_distrito;
					id_agencia = this.state.catalogoDistritos[0].id_agencia;
			...
			// datos del delito segun seleccionado
			id_distrito = this.state.catalogoDistritos[0].id_distrito;
			...
			// Creación de pre registro
			axios
				.post("https://" + ip_address + ":" + puerto + "/api/PreRegistros/GenerarPreRegistro", datos_pre_denuncia)
				.then((response) => {
					console.log(response);
					let respuesta_pre_registro = response.data;
					datos_recabados.push({
						preregistro_id: respuesta_pre_registro.prereg,
					});
					var datos_denuncia = {
						DistritoId: id_distrito,
						PRegistroId: respuesta_pre_registro.prereg,
						StatusAnonimo: parametros.swAnonimo,
						TipoPersona: parametros.selClasPersona,
						RFC: parametros.txtRFC,
						RazonSocial: parametros.txtRazonSocial,
						Nombre: parametros.txtNombre,
						ApellidoPaterno: parametros.txtApPaterno,
						ApellidoMaterno: parametros.txtApMaterno,
						StatusAlias: parametros.txtAlias,
						FechaNacimiento: parametros.dateFNacimiento,
						EntidadFederativa: parametros.selEntidadFederativa,
						CURP: parametros.txtCurp,
						Sexo: parametros.selSexo,
						EstadoCivil: parametros.selEstadoCivil,
						Genero: parametros.selLGBT,
						Telefono1: parametros.txtnumTel1,
						Telefono2: parametros.txtnumTel2,
						Correo: parametros.emailCorreo,
						Medionotificacion: parametros.selNotificacion,
						Nacionalidad: parametros.txtNacionalidad,
						Ocupacion: parametros.selOcupacion,
						NivelEstudio: parametros.selNivelEstudios,
						Lengua: parametros.selLengua,
						Religion: parametros.selReligion,
						Discapacidad: parametros.swDiscapacidad,
						TipoDiscapacidad: parametros.selDiscapacidad,
						DatosProtegidos: parametros.datosProtegidos,
						Parentesco: parametros.parentesco,
						Edad: parametros.txtNumEdad,
						Relacion: parametros.relacion,
						EstadoId: parametros.selEstado,
						MunicipioId: parametros.selMunicipio,
						LocalidadId: parametros.selLocalidad,
					};

					axios
						.post("https://" + ip_address + ":" + puerto + "/api/PreAtenciones/Crear", datos_denuncia)
						.then((response) => {
							console.log(response);
							let respuesta_atenciones = response.data;
							datos_recabados.push({
								patencion_id: respuesta_atenciones.idpatencion,
								persona_id: respuesta_atenciones.personaid,
								direccion_personal: respuesta_atenciones.direccionPersonal,
								pre_rap_id: respuesta_atenciones.rap,
							});
							let datos_pre_hecho = {
								distritoId: id_distrito,
								PAtencionId: respuesta_atenciones.idpatencion,
								FechaHoraSuceso: new Date().toISOString(),
								FechaHoraSuceso2: new Date().toISOString(),
								FechaReporte: new Date().toISOString(),
								rbreve: this.state.txtDescHechos,
							};

							axios
								.post("https://" + ip_address + ":" + puerto + "/api/PreHecho/Crear", datos_pre_hecho)
								.then((response) => {
									console.log(response);
									let respuesta_prehecho = response.data;
									datos_recabados.push({
										hecho_id: respuesta_prehecho.idHecho,
									});
									let datos_RDH = {
										DistritoId: id_distrito,
										PHechoId: respuesta_prehecho.null, //! checar esto cuando haya internet XD
										DelitoId: null,
										TipoFuero: null,
										TipoDeclaracion: null,
										ResultadoDelito: null,
										GraveNoGrave: null,
										Equiparado: null,
										IntensionDelito: null,
										ViolenciaSinViolencia: null,
										Tipo: null,
										Concurso: null,
										ClasificaOrdenResult: null,
										ArmaFuego: null,
										ArmaBlanca: null,
										Hipotesis: null,
									};
									...
									axios
										.post("https://" + ip_address + ":" + puerto + "/api/PreHecho/Crear", datos_RDH)
										.then((response) => {
											console.log(response);
											let respuesta_rdh = response.data;
											datos_recabados.push({
												rdh: respuesta_rdh.rdh,
											});
											let datos_cita = {
												PRegistroId: null,
												Fecha: null,
											...
											axios
												.post("https://" + ip_address + ":" + puerto + "/api/PreCitas/Crear", datos_cita)
												.then((response) => {
													console.log(response);
													let respuesta_citas = response.data;
													datos_recabados.push({
														cita_id: respuesta_citas.idcita,
													});
	...
	render() {
		return (
			<div className="d-flex justify-content-md-end" style={{ backgroundColor: "#f4f4f4", minHeight: "100vh" }}>
				{/* <button className="btn btn-dark fs-6" onClick={this.enviarCorreo} style={{ marginTop: "10px" }}>
						...
						<div className="container-fluid h-100 pt-5 px-3" style={{ backgroundColor: "#f4f4f4" }}>
							<div className="row">
								<div className="col mb-2">
									...
									<br />
									<small className="text-danger">Servicio disponible únicamente para CESI Pachuca y Tulancingo</small>
								</div>
							</div>
							<div className="row">
								<div className="col-md-6">
									...
									<div className="form-group mb-3">
										<label>
											<input
												onChange={this.checkAnonimo.bind(this)}
												checked={this.state.swAnonimo === true}
												type="checkbox"
												id="swAnonimo"
												name="swAnonimo"
									...
									<div className="form-group mb-3">
										<label>
											<input
												...
												disabled={this.state.swAnonimo === true}
												type="checkbox"
												id="raPersona"
												name="raPersona"
									...
									<div className="form-group mb-3">
										...
										<select
											...
											ref={(selClasPersona) => (this.inputSelClasPersona = selClasPersona)}
										>
											{this.state.catalogoClasPersona
												? this.state.catalogoClasPersona.map((datos) => (
														<option key={datos.idClasificacionPersona} value={datos.nombre}>
															{datos.nombre}
														</option>
												  ))
												: "Cargando..."}
									...
									<span hidden={this.state.raPersona === "Fisica" || this.state.swAnonimo === true}>
										<small className="text-secondary">
											...
											<small
												...
												hidden={this.state.raPersona === "Fisica" || this.state.swAnonimo === true}
											>
												*
											</small>
										</small>
									...
									>
										<input
											required={this.state.raPersona === "Moral" || this.state.swAnonimo === false}
											onChange={this.handlerOnChange}
											id="txtRFC"
											type="text"
											...
											name="txtRFC"
											value={
												this.state.swAnonimo === true || this.state.raPersona === "Fisica" ? " " : this.state.txtRFC
											}
									...
									>
										<input
											required={this.state.raPersona === "Moral" || this.state.swAnonimo === false}
											onChange={this.handlerOnChange}
											disabled={this.state.raPersona === "Fisica" || this.state.swAnonimo === true}
											id="txtRazonSocial"
											type="text"
											className="form-control w-100"
											name="txtRazonSocial"
											value={
												this.state.swAnonimo === true || this.state.raPersona === "Fisica"
													? " "
													: this.state.txtRazonSocial
											}
											ref={(txtRazonSocial) => (this.inputTxtRazonSocial = txtRazonSocial)}
										/>
									</div>
									...
									<span>
										<small className="text-secondary">
											Nombre:{" "}
											<small className="text-secondary" hidden={this.state.swAnonimo === true}>
												*
											</small>
										</small>
									</span>
									<div className="form-group mb-3">
										<input
											required={this.state.raPersona === "Fisica" || this.state.swAnonimo === false}
											onChange={this.handlerOnChange}
											disabled={this.state.swAnonimo === true}
											id="txtNombre"
											type="text"
											className="form-control w-100"
											name="txtNombre"
											value={this.state.swAnonimo === true ? " " : this.state.txtNombre}
									...
									<div className="form-group mb-3">
										<input
											...
											className="form-control w-100"
											name="txtApPaterno"
											value={this.state.swAnonimo === true ? " " : this.state.txtApPaterno}
											ref={(txtApPaterno) => (this.inputTxtApPaterno = txtApPaterno)}
										/>
									</div>
									...
									<div className="form-group mb-3">
										<input
											...
											className="form-control w-100"
											name="txtApMaterno"
											value={this.state.swAnonimo === true ? " " : this.state.txtApMaterno}
											ref={(txtApMaterno) => (this.inputTxtApMaterno = txtApMaterno)}
										/>
									</div>
									...
									<div className="form-group mb-3">
										<input
											...
											className="form-control w-100"
											name="txtAlias"
											value={this.state.swAnonimo === true ? " " : this.state.txtAlias}
									...
									<div className="row mb-3 align-items-center">
										<div className="col-md-6">
											<span>
												...
												<br />
												<small className="text-danger" hidden={this.state.raPersona === "Fisica"}>
										...
										<div className="col-md-6">
											<input
												onChange={this.handleChange_edad}
												type="date"
												id="dateFNacimiento"
												className="form-control w-100"
												name="dateFNacimiento"
												value={this.state.dateFNacimiento}
												ref={(dateFNacimiento) => (this.inputDateFNacimiento = dateFNacimiento)}
											/>
										</div>
									</div>
										...
										>
											...
											{this.state.catalogoGenero
												? this.state.catalogoGenero.map((datos) => (
														<option key={datos.idGenero} value={datos.nombre}>
															{datos.nombre}
														</option>
												  ))
												: "Cargando..."}
								...
								<div className="col-md-6">
									...
									<span>
										<small className="text-secondary">
											...
											<small
												className="text-secondary"
												hidden={this.state.swAnonimo === true || this.state.raPersona === "Moral"}
											>
												*
											</small>
										</small>
									...
									<div className="form-group mb-3">
										<select
											...
											ref={(selEntidadFederativa) => (this.inputSelEntidadFederativa = selEntidadFederativa)}
										>
											<option value="">Seleccione</option>
											{this.state.catalogoEstado
												? this.state.catalogoEstado.map((datos) => (
														<option key={datos.idEstado} value={datos.nombre}>
															{datos.nombre}
														</option>
												  ))
												: "Cargando..."}
									...
									<div className="form-group mb-3">
										<select
											...
											ref={(selIdentificacion) => (this.inputSelIdentificacion = selIdentificacion)}
										>
											<option value="">Seleccione</option>
											{this.state.catalogoDocIdentificacion
												? this.state.catalogoDocIdentificacion.map((datos) => (
														<option key={datos.idDocIdentificacion} value={datos.nombre}>
															{datos.nombre}
														</option>
												  ))
												: "Cargando..."}
									...
									<div className="form-group mb-3">
										<input
											...
											type="file"
											accept="image/*"
											onChange={this.cargarDocumento.bind(this)}
											disabled={this.state.swAnonimo === true}
											className="form-control w-100"
											id="inputGroupFile02"
										/>
									...
									<div className="form-group mb-3">
										<input
											...
											className="form-control w-100"
											name="txtCurp"
											value={this.state.swAnonimo === true ? " " : this.state.txtCurp}
									...
									<div className="form-group mb-3" hidden={this.state.swAnonimo === false}>
										<input
											...
											ref={(emailCorreo) => (this.inputEmailCorreo = emailCorreo)}
										/>
									</div>
								</div>
							</div>
						...
						<div className="container-fluid h-100 pt-5 px-3" style={{ backgroundColor: "#f4f4f4" }}>
							<div className="col mb-2">
								...
								<small className="text-danger">
									Servicio únicamente disponible para CESIS Pachuca, Tula, Tulancingo e Ixmiquilpan
								</small>
							</div>
								...
								<div className="col-md-6">
									...
									<div className="form-group mb-3">
										<select
											...
											ref={(selNotificacion) => (this.inputSelNotificacion = selNotificacion)}
										>
											<option value="">Seleccione...</option>
											<option value="Correo Electronico">Correo Electronico</option>
											<option value="Domicilio" disabled={this.state.swAnonimo === true}>
												Domicilio
											</option>
											<option value="Telefono" disabled={this.state.swAnonimo === true}>
												Telefono
											</option>
										</select>
									</div>
									...
									<div className="form-group mb-3">
										<input
											...
											type="number"
											name="txtnumTel1"
											value={this.state.swAnonimo === true ? " " : this.state.txtnumTel1}
									...
									<div className="form-group mb-3">
										<input
											...
											type="number"
											name="txtnumTel2"
											value={this.state.swAnonimo === true ? " " : this.state.txtnumTel2}
									...
									<div className="form-group mb-3">
										<input
											...
											type="text"
											name="emailCorreo"
											value={this.state.swAnonimo === true ? " " : this.state.emailCorreo}
											ref={(emailCorreo) => (this.inputEmailCorreo = emailCorreo)}
										/>
									</div>
									...
									<div className="form-group mb-3">
										<select
											required={this.state.raPersona === "Moral"}
											onChange={this.handlerOnChange}
											disabled={this.state.swAnonimo === true}
											className="form-select"
											id="txtNacionalidad"
											...
											value={this.state.swAnonimo === true ? " " : this.state.txtNacionalidad}
											ref={(txtNacionalidad) => (this.inputTxtNacionalidad = txtNacionalidad)}
										>
											<option value="">Seleccione</option>
											{this.state.catalogoNacionalidad
												? this.state.catalogoNacionalidad.map((datos) => (
														<option key={datos.idNacionalidad} value={datos.nombre}>
															{datos.nombre}
														</option>
												  ))
												: "Cargando..."}
									...
									<div
										className="form-group mb-3"
										hidden={this.state.raPersona === "Moral" || this.state.swAnonimo === true}
									>
										{/* Select Estado Civil*/}
										<select
											required={this.state.raPersona === "Fisica" || this.state.swAnonimo === false}
											onChange={this.handlerOnChange}
											disabled={this.state.raPersona === "Moral" || this.state.swAnonimo === true}
											className="form-select"
											id="selEstadoCivil"
											name="selEstadoCivil"
											value={this.state.selEstadoCivil}
											ref={(selEstadoCivil) => (this.inputSelEstadoCivil = selEstadoCivil)}
										>
											<option value="">Seleccione...</option>
								...
								<div className="col-md-6">
									...
									<div className="form-group mb-3">
										<select
											...
											ref={(selOcupacion) => (this.inputSelOcupacion = selOcupacion)}
										>
											<option value="">Seleccione</option>
											{this.state.catalogoOcupacion
												? this.state.catalogoOcupacion.map((datos) => (
														<option key={datos.idOcupacion} value={datos.nombre}>
															{datos.nombre}
														</option>
												  ))
												: "Cargando..."}
									...
									<div className="form-group mb-3">
										...
										<select
											...
											ref={(selNivelEstudios) => (this.inputSelNivelEstudios = selNivelEstudios)}
										>
											<option value="">Seleccione</option>
											{this.state.catalogoNivelEstudios
												? this.state.catalogoNivelEstudios.map((datos) => (
														<option key={datos.idNivelestudios} value={datos.nombre}>
															{datos.nombre}
														</option>
												  ))
												: "Cargando..."}
										...
										>
											...
											{this.state.catalogoLengua
												? this.state.catalogoLengua.map((datos) => (
														<option key={datos.idLengua} value={datos.nombre}>
															{datos.nombre}
														</option>
												  ))
												: "Cargando..."}
									...
									>
										<select
											...
											ref={(selReligion) => (this.inputSelReligion = selReligion)}
										>
											<option value="">Seleccione</option>
											{this.state.catalogoReligion
												? this.state.catalogoReligion.map((datos) => (
														<option key={datos.idReligion} value={datos.nombre}>
															{datos.nombre}
														</option>
												  ))
												: "Cargando..."}
									...
									>
										<div className="col-md-6">
											<input
												...
												onChange={this.checkLGBT.bind(this)}
												checked={this.state.swLGBT === true}
												disabled={this.state.swAnonimo === true}
												ref={(swLGBT) => (this.inputSwLGBT = swLGBT)}
											/>
											<label className="form-label" onClick={this.checkLGBT.bind(this)} style={{ fontSize: "14px" }}>
												&nbsp;¿Pertenece a la comunidad LGBTTTQA?
											</label>
										</div>
										{/* Select LGBT*/}
										<div className="col-md-6">
											<select
												required={this.state.swLGBT === true}
												onChange={this.handlerOnChange}
												disabled={this.state.swLGBT === false}
												className="form-select"
												id="selLGBT"
												name="selLGBT"
									...
									>
										<div className="col-md-6">
											<input
												...
												id="swDiscapacidad"
												name="swDiscapacidad"
												onChange={this.checkDisc.bind(this)}
												checked={this.state.swDiscapacidad === true}
												value={this.state.swDiscapacidad}
												disabled={this.state.swAnonimo === true}
												ref={(swDiscapacidad) => (this.inputSwDiscapacidad = swDiscapacidad)}
											/>
											<label className="form-label" onClick={this.checkDisc.bind(this)} style={{ fontSize: "14px" }}>
										...
										<div className="col-md-6">
											<select
												required={this.state.swDiscapacidad === true}
												onChange={this.handlerOnChange}
												disabled={this.state.swDiscapacidad === false}
												className="form-select"
												id="selDiscapacidad"
												name="selDiscapacidad"
												value={this.state.selDiscapacidad}
												ref={(selDiscapacidad) => (this.inputSelDiscapacidad = selDiscapacidad)}
											>
												<option value="">Seleccione</option>+
												{this.state.catalogoDiscapacidad
													? this.state.catalogoDiscapacidad.map((datos) => (
															<option key={datos.idDiscapacidad} value={datos.nombre}>
																{datos.nombre}
															</option>
													  ))
													: "Cargando..."}
						...
						<div className="container-fluid h-100 pt-5 px-3" style={{ backgroundColor: "#f4f4f4" }}>
							...
							<div className="row">
								...
								<div className="col-md-6">
									{/* Delito */}
									<label>
										<small> Delito Cometido: *</small>
									</label>
									{/* Select Delito*/}
									<div className="form-group mb-3">
										<select
											required={this.state.selDelito === true}
											onChange={this.handlerOnChange}
											className="form-select"
											id="selDiscapacidad"
											name="selDiscapacidad"
											value={this.state.selDelito}
											ref={(selDelito) => (this.inputSelDelito = selDelito)}
										>
											<option value="">Seleccione</option>+
											{this.state.catalogoDelitos
												? this.state.catalogoDelitos.map((datos) => (
														<option key={datos.idDelito} value={datos.idDelito}>
															{datos.nombre}
														</option>
												  ))
												: "Cargando..."}
									...
									<div className="form-group mb-3">
										<textarea
											...
											className="form-control w-100"
											name="txtDescHechos"
											value={this.state.txtDescHechos}
											ref={(txtDescHechos) => (this.inputTxtDescHechos = txtDescHechos)}
										></textarea>
									</div>
									...
									<div className="row mb-3 align-items-center">
										<div className="col">
											<input
												...
												ref={(dateFSuceso) => (this.inputDateFSuceso = dateFSuceso)}
											/>
										</div>
									</div>
									...
									<div className="row mb-3 align-items-center">
										<div className="col">
											<input
												...
												ref={(timeHoraSuceso) => (this.inputTimeHoraSuceso = timeHoraSuceso)}
											/>
										</div>
									</div>
									...
									<div className="form-group mb-3">
										<input
											...
											type="text"
											className="form-control w-100"
											ref={(txtEntCalle1) => (this.inputTxtEntCalle1 = txtEntCalle1)}
										/>
									</div>
									...
									<div className="form-group mb-3">
										<input
											...
											type="text"
											className="form-control w-100"
											ref={(txtEntCalle2) => (this.inputTxtEntCalle2 = txtEntCalle2)}
										/>
									</div>
									...
									<div className="form-group mb-3">
										<input
											...
											type="text"
											className="form-control w-100"
											ref={(txtReferencias) => (this.inputTxtReferencias = txtReferencias)}
										/>
									</div>
								</div>
								...
								<div className="col-md-6">
									...
									<div className="form-group mb-3">
										<select
											required
											onChange={(this.handlerOnChange, this.handler_estado.bind(this))}
											className="form-select"
											id="selEstado"
											name="selEstado"
									...
									<div className="form-group mb-3">
										<select
											...
											ref={(selMunicipio) => (this.inputSelMunicipio = selMunicipio)}
											aria-label="municipio"
										>
											<option value="">Seleccione</option>
											{this.state.catalogoMunicipio ? (
												this.state.catalogoMunicipio.map((datos) => (
													<option key={datos.idMunicipio} value={datos.idEstado}>
														{datos.nombre}
													</option>
												))
											) : (
									...
									<div className="form-group mb-3">
										<select
											...
											id="selLocalidad"
											name="selLocalidad"
											ref={(selLocalidad) => (this.inputSelLocalidad = selLocalidad)}
											aria-label="municipio"
										>
											<option value="">Seleccione</option>
											{this.state.catalogoLocalidad ? (
												this.state.catalogoLocalidad.map((datos) => (
													<option key={datos.idLocalidad} value={datos.nombre}>
														{datos.nombre}
													</option>
												))
											) : (
									...
									<div className="form-group mb-3">
										<input
											...
											ref={(txtCodPostal) => (this.inputTxtCodPostal = txtCodPostal)}
										/>
									</div>
									...
									<div className="form-group mb-3 align-items-center">
										<select
											...
											onChange={this.handlerOnChange}
											className="form-select"
											id="selAgenciaAVisitar"
											name="selAgenciaAVisitar"
											value={this.state.selAgenciaAVisitar}
											ref={(selAgenciaAVisitar) => (this.inputSelAgenciaAVisitar = selAgenciaAVisitar)}
											aria-label="agencia a visitar"
										>
											<option value="">Seleccione</option>
									...
									<div className="row mb-3 align-items-center">
										...
										<div className="col-md-6">
											<input
												required
												onChange={(this.handlerOnChange, this.checarDiaSemana.bind(this))}
												type="date"
												id="dateFechaCita"
												size="60"
												min="2021-09-17"
												...
												name="dateFechaCita"
												value={this.state.dateFechaCita}
												ref={(dateFechaCita) => (this.inputDateFechaCita = dateFechaCita)}
									...
									<div className="row mb-3 align-items-center">
										...
										<div className="col-md-6">
											<select
												...
												id="selHorarioCita"
												name="selHorarioCita"
												value={this.state.selHorarioCita}
												ref={(selHorarioCita) => (this.inputSelHorarioCita = selHorarioCita)}
												aria-label="Horario de cita"
											>
												{this.state.horariosDisponibles ? (
													this.state.horariosDisponibles.map((datos) => (
														<option key={datos.hora} value={datos.hora}>
															{datos.hora}
														</option>
													))
												) : (
							...
							<div className="row">
								<div className="col d-grid gap-2 d-md-flex justify-content-md-end mb-3">
									...
									<button className="btn btn-dark fs-6" onClick={this.enviar.bind(this)} style={{ marginTop: "10px" }}>
										FINALIZAR
									</button>
								</div>
							</div>
									...
									<div className="card mb-5">
										<div className="card-body text-center">
											...
											<p className="card-text">
												En la brevedad, recibirá una notificación por el medio seleccionado con su numero de cita y
												formato de solicitud de denuncia
											</p>
											<button className="btn btn-dark fs-6" onClick={this.recargar.bind(this)}>
												Registrar nueva denuncia
											</button>
											<button className="btn btn-dark fs-6" onClick={this.enviarCorreo}>
												Registrar nueva denuncia
											</button>
										</div>
									</div>
